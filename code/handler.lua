...
local function get_secret_from_vault(vault_addr, vault_token, data_path, key)
  ...
  if not res then
    kong.log.err("Failed to fetch secret from Vault: ", err or "unknown error")
    return nil, "Failed to fetch secret from Vault: " .. (err or "unknown error")
  end
  ...
  if not value then
    kong.log.err("Key '", key, "' not found in Vault path '", data_path, "'")
    return nil, "Key '" .. key .. "' not found in Vault path '" .. data_path .. "'"
  end
...
function VaultAppRoleHandler:access(config)
  -- Lấy Vault token
  local vault_token, err = get_vault_token(
    config.vault_addr,
  ...
  for header_name, header_value in pairs(headers) do
    if type(header_value) == "string" then
      ...
      if path and key then
        ...
        local secret_value, secret_err = get_secret_from_vault(
          config.vault_addr,
          vault_token,
          path,
          key
        ...
        if secret_value then
          ...
          kong.log.info("Replaced header '", header_name, "' with Vault secret")
        else
          kong.log.err("Failed to get secret for header '", header_name, "': ", secret_err)
        end
      end
    end
  end
  ...
  if config.api_key_header then
    ...
    local expected_key
    if path and key then
      expected_key, err = get_secret_from_vault(
        config.vault_addr,
        vault_token,
        path,
        key
